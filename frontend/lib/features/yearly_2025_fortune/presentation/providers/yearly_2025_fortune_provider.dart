...
class Yearly2025Fortune extends _$Yearly2025Fortune {
  ...
  Future<Yearly2025FortuneData?> build() async {
    ...
    if (_isAnalyzing && _analyzeStartTime != null &&
      ...
      print('[Yearly2025Fortune] ⚠️ _isAnalyzing 타임아웃 리셋 (${_analyzeTimeout.inMinutes}분 초과)');
      _isAnalyzing = false;
    ...
    if (!SupabaseService.isConnected) {
      if (_offlineRetryCount >= _maxOfflineRetries) {
        print('[Yearly2025Fortune] 오프라인 재시도 초과 ($_offlineRetryCount/$_maxOfflineRetries)');
        _offlineRetryCount = 0;
        return null;
      }
      _offlineRetryCount++;
      print('[Yearly2025Fortune] 오프라인 모드 - 3초 후 재시도 ($_offlineRetryCount/$_maxOfflineRetries)');
