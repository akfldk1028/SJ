/// 24절기 시각 테이블 확장 버전
///
/// 이 파일은 기존 solar_term_table.dart를 대체하며,
/// 1900-2100년 전체 범위의 절기 데이터를 제공합니다.
///
/// 데이터 소스:
/// - 2020-2030년: 한국천문연구원 공식 데이터
/// - 그 외 연도: 천문학적 계산값 (Jean Meeus 알고리즘, 오차 ±60분 이내)
library;
...
/// 절기 테이블 캐시
final Map<int, Map<String, SolarTermData>> _cache = {};

/// 공식 데이터가 있는 연도 범위
const int _officialDataStartYear = 2020;
const int _officialDataEndYear = 2030;

/// 지원 연도 범위
const int supportedStartYear = 1900;
const int supportedEndYear = 2100;

/// 연도별 절기 데이터 조회
///
/// [year]: 조회할 연도 (1900-2100)
/// 반환: 해당 연도의 24절기 데이터 맵
///
/// 사용 예:
/// ```dart
/// final terms2024 = getSolarTermsForYear(2024);
/// final ipchun = terms2024['ipchun'];
/// print('2024년 입춘: ${ipchun?.dateTime}');
/// ```
Map<String, SolarTermData> getSolarTermsForYear(int year) {
  if (year < supportedStartYear || year > supportedEndYear) {
    throw ArgumentError('지원 연도 범위: $supportedStartYear-$supportedEndYear (요청: $year)');
  }
  ...
  // 공식 데이터 범위면 공식 데이터 사용
  if (year >= _officialDataStartYear && year <= _officialDataEndYear) {
    final officialData = _officialSolarTermTable[year];
    if (officialData != null) {
      _cache[year] = officialData;
      return officialData;
    }
  }

  // 계산값 생성
  final calculatedData = _generateSolarTermData(year);
  _cache[year] = calculatedData;
  return calculatedData;
}

/// 특정 연도의 절기 데이터 계산
Map<String, SolarTermData> _generateSolarTermData(int year) {
  final terms = <String, SolarTermData>{};
  final calculated = SolarTermCalculator.calculateYearTerms(year);

  for (int i = 0; i < 24; i++) {
    final key = SolarTermCalculator.termKeys[i];
    final name = SolarTermCalculator.termNames[i];
    terms[key] = SolarTermData(calculated[key]!, name);
  }
...
///
/// [date]: 조회할 날짜
/// 반환: (절기키, 절기데이터) 또는 null
(String, SolarTermData)? findSolarTermForDate(DateTime date) {
  final year = date.year;
  final terms = getSolarTermsForYear(year);

  // 이전 절기 찾기 (날짜 이전의 가장 최근 절기)
  String? foundKey;
  SolarTermData? foundData;

  for (final entry in terms.entries) {
    if (entry.value.dateTime.isBefore(date) ||
        entry.value.dateTime.isAtSameMomentAs(date)) {
      if (foundData == null || entry.value.dateTime.isAfter(foundData.dateTime)) {
        foundKey = entry.key;
        foundData = entry.value;
      }
    }
  }
  ...
  // 해당 연도에 없으면 이전 연도 마지막 절기
  if (year > supportedStartYear) {
    final prevYearTerms = getSolarTermsForYear(year - 1);
    final dongji = prevYearTerms['dongji'];
    if (dongji != null) {
      return ('dongji', dongji);
    }
  }
...
/// 캐시 초기화 (테스트용)
void clearSolarTermCache() {
  _cache.clear();
}

// ============================================================
// 한국천문연구원 공식 데이터 (2020-2030년)
// ============================================================

final Map<int, Map<String, SolarTermData>> _officialSolarTermTable = {
  // ========== 2020년 (한국천문연구원 데이터) ==========
  2020: {
    ...
    'chunbun': SolarTermData(DateTime(2020, 3, 20, 12, 50), '춘분'),
  ...
  2024: {
    ...
    'chubeun': SolarTermData(DateTime(2024, 9, 22, 21, 43), '추분'),
    'hanro': SolarTermData(DateTime(2024, 10, 8, 3, 0), '한로'),
    'sanggang': SolarTermData(DateTime(2024, 10, 23, 7, 14), '상강'),
    'ipdong': SolarTermData(DateTime(2024, 11, 7, 7, 19), '입동'),
...
/// @deprecated 대신 getSolarTermsForYear()를 사용하세요.
/// 기존 코드와의 호환성을 위해 유지
@Deprecated('getSolarTermsForYear()를 사용하세요')
Map<int, Map<String, SolarTermData>> get solarTermTable {
  // 기존 데이터(2020-2030)만 반환
  return Map.unmodifiable(_officialSolarTermTable);
}
