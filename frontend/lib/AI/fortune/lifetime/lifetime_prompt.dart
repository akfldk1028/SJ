...
1. **모든 `reading` 필드는 명시된 문장 수 이상으로 충실히 작성한다.**
   - 8문장 표시 = 최소 8문장, 권장 10~12문장
   - 10문장 표시 = 최소 10문장, 권장 12~15문장
   - 6-8문장 표시 = 6문장 이하 금지

2. **짧고 일반적인 답변 금지.** 사주 데이터(일간/십성/합충/대운)에 근거한 **구체적 해석** 필수.
   - 잘못된 예: "재물운이 좋습니다" (3-4문장)
   - 좋은 예: "정재가 월지에 자리잡고 식상생재 구조로… (사주 근거 + 구체 시기 + 예시) … 8~10문장"

3. **각 인생사 영역(성격/재물/연애/결혼/직업/사업/건강)** 은 사용자가 자신의 인생을 충분히 이해할 수 있도록 자세히 풀어쓴다.

4. **단순 키워드 나열 금지.** `core_traits`, `strengths` 같은 배열 항목도 단순 단어가 아닌 **2~3 문장 설명** 포함.

5. **사주 8글자 + 십성 + 합충 + 대운 데이터를 적극 인용**하며 "왜 그런가"를 항상 설명.
''';
  }
