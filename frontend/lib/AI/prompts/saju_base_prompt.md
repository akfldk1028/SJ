...
```
● 결론: Output/Input 규칙 유지하면서 해결 가능                                                       
                                                                                                     
  제약조건 체크                                                                                      
  ┌────────────────────┬────────────┬────────────────────────────────────────┐                       
  │        제약        │ 유지 가능? │                  방법                  │                       
  ├────────────────────┼────────────┼────────────────────────────────────────┤                       
  │ Output JSON 스키마 │ ✅         │ Phase별 부분 생성 → 병합하면 동일 구조 │                       
  ├────────────────────┼────────────┼────────────────────────────────────────┤                       
  │ Input 데이터 구조  │ ✅         │ SajuInputData 그대로 사용              │                       
  ├────────────────────┼────────────┼────────────────────────────────────────┤                       
  │ 다른 팀원 영향     │ ✅ 없음    │ 파싱 코드 변경 불필요                  │                       
  └────────────────────┴────────────┴────────────────────────────────────────┘                       
  ---                                                                                                
  변경되는 것 vs 유지되는 것                                                                         
  ...
  ┌─────────────────────────────────────────────────────────────┐
  │ 변경 (프롬프트 내부만) - 2026-01-21 적용 완료                │
  ├─────────────────────────────────────────────────────────────┤
  │ • summary: 30문장 → 10문장                                   │
  │ • 각 reading 필드: 15-20문장 → 6-10문장                      │
  │ • life_cycles 각 시기: 10문장 → 5문장                        │
  │ • daeun_detail.cycles: 8개×10문장 → 3개×5문장 (81% 감소)    │
  │ • 총 예상 토큰: ~8,000 → ~3,000 (62% 감소)                   │
  └─────────────────────────────────────────────────────────────┘                                    
                                                                                                     
  ---                                                                                                
  Phase 분할 시 스키마 유지 방법                                                                     
                                                                                                     
  Phase 1 (GPT-5.2) → { wonGuk, sipsung, hapchung, personality, lucky }                              
  Phase 2 (GPT-mini) → { wealth, career, business, love, marriage }                                  
  Phase 3 (GPT-mini) → { health, sinsal_gilseong, daeun_detail }                                     
  Phase 4 (GPT-5.2) → { summary, life_cycles, peak_years, modern }                                   
  ...
  {
    "personality": {
      "reading": "경금 일간은... (20문장 장문)"
    },
    "daeun_detail": {
      "cycles": [/* 8개 대운 × 10문장 = 80문장 */]
    }
  }
  ...
  {
    "personality": {
      "reading": "경금 일간은... (8문장 핵심)"
    },
    "daeun_detail": {
      "cycles": [/* 3개 대운 × 5문장 = 15문장 */]
    }
  }                                                                                                  
                                                                                                     
  → 키 구조 동일, 내용만 간결                                                                        
  → Output 팀원의 파싱 코드 변경 불필요!                                                             
