...
### 해결 방안 검토 필요
- [ ] GPT-5.2 모델 유지하면서 타임아웃 해결 방법 검토
- [ ] Background Task + Polling 패턴 재검토
- [ ] 기타 대안 검토
...
1. **saju_base 타임아웃 해결** ← 우선순위 높음
2. 채팅 테스트 후 토큰 사용량 확인
3. 만세력 계산 추가 검증 (다른 생년월일 테스트)
4. Phase 27 (진공/반공 신살 추가) - 공망의 충/합 위치
5. 시간 모름 처리 개선 - 삼주(三柱) 분석 모드
6. 필요시 추가 버그 수정
