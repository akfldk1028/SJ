...
| `intervalAd` | **6번째** 메시지마다 | **네이티브** (AdNativeBubble) | 클릭 시 **30,000** / 노출만 **0** | **클릭 시에만** | 가능 |
| (인라인) | **6번째** 메시지마다 | **정적 네이티브** (ChatAdWidget) | **0** | - | - |
...
**인터벌 광고 (AdNativeBubble, 채팅 리스트 끝)**
```
┌──────────────────────────┐
│     [네이티브 광고]        │  ← 채팅 리스트 trailingWidget
└──────────────────────────┘
  "관심 있는 광고를 살펴보시면 대화를 이어갈 수 있어요"
  → 광고 클릭 → +30,000 토큰 → 채팅 자동 재개
  → 스킵 → 토큰 없이 대화 계속 가능
```

**인라인 정적 광고 (ChatAdWidget, 메시지 사이)**
```
  [유저 메시지]
  [AI 응답]
  ────────────────
  [네이티브 광고]  ← AI 응답 뒤에만 삽입
  ────────────────
  [유저 메시지]
  → 순수 광고 수익 (토큰 보상 없음)
```
...
| 설정 | 값 | 설명 |
|------|------|------|
| `inlineAdMessageInterval` | **6** | 6메시지(=3교환)마다 |
...
```
메시지 1~6: 첫 3교환 (광고 없음)
메시지 6 이후:  인라인 광고 (ChatAdWidget, AI 응답 뒤에만)
              + 인터벌 광고 (AdNativeBubble, 클릭 시 +30,000 토큰)
메시지 7~12: 다음 3교환
메시지 12:  다시 인라인/인터벌 광고
...
```
...
| 인터벌 광고 UI | `ConversationalAdWidget` trailing | `AdNativeBubble` trailing + 안내 문구 |
| 인라인 광고 | `ChatAdWidget` (메시지 사이) | 동일 (AI 응답 뒤에만 삽입으로 수정) |
...
| `inlineAdMessageInterval` | 2 | **6** |
| `inlineAdMinMessages` | 2 | **6** |
...
**4. AdMob 정책 준수**
- 네이티브 광고 안내 문구: "관심 있는 광고를 살펴보시면 대화를 이어갈 수 있어요"
- "클릭하세요" 직접 유도 금지
