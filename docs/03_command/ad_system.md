...
| 쿼터 초과 판단 | `chatting_tokens >= (daily_quota + bonus_tokens + rewarded_tokens_earned)` |
...
|--------|------|----------|----------|--------------|------|
| `tokenDepleted` | 토큰 100% 소진 | **2버튼 선택** | 영상 **35,000** / 네이티브 **21,000** | 영상 시청 or 광고 보기 | 불가 |
| `tokenNearLimit` | 토큰 80%+ | ~~비활성화~~ | 0 | `warningRewardTokens=0` | - |
| `intervalAd` | **2번째** 메시지마다 | **네이티브** | impression **1,500** + click **+1,500** | **자동** (impression) | 가능 |
...
## 4. 인라인 광고 설정 (`AdStrategy`) - v27

| 설정 | 값 | 설명 |
|------|------|------|
| `inlineAdMessageInterval` | **2** | 2번째 메시지마다 (매 대화) |
| `inlineAdMaxCount` | **9999** | 무제한 |
| `inlineAdMinMessages` | **2** | 2번째 메시지부터 광고 가능 |
...
```
메시지 1~2: 첫 대화
메시지 2:   첫 번째 광고 → impression 1,500 토큰 자동 지급
메시지 3~4: 다음 대화
메시지 4:   두 번째 광고 → impression 1,500 토큰
...
→ 무제한 반복, 할수록 이득 (impression 수입 > 토큰 비용)
```

### v27 핵심 변경: 즉시 서버 저장

```
Before (v26): impression → state만 → "대화 재개" 버튼 → RPC (미클릭 시 유실!)
After (v27):  impression → state + _saveBonusToServer() 즉시 → DB 즉시 반영
```
...
**2. 채팅 내 광고 빈도 조정 (v27 최종)**

| 설정 | 원래 | SONNY 제안 | DK-CC 최종 |
|------|------|-----------|-----------|
| `inlineAdMessageInterval` | 5 | 10 | **2** |
| `inlineAdMinMessages` | 5 | 10 | **2** |
| `inlineAdMaxCount` | 10 | 10 | **9999** |
| `interstitialMinInterval` | 60초 | 30초 | **0초** |

**3. 토큰 보상 구조 (v27 최종)**

| 구분 | 토큰 | 왕복 대화 | 설명 |
|------|------|----------|------|
| 초기 무료 | 20,000 | ~3번 | 기본 제공 (daily_quota) |
| 🎬 영상 광고 | 35,000 | 5번 | 토큰 소진 시 보상형 영상 |
| 📋 네이티브 광고 (소진 선택) | 21,000 | 3번 | 토큰 소진 시 네이티브 선택 |
| 👁️ impression 보상 | 1,500 | 0.2번 | 2메시지마다 자동 |
| 👆 클릭 보너스 | +1,500 | +0.2번 | 클릭 시 추가 |
...
## 8. v27 수정 요약 (2026-02-01)

### 코드 변경

| 파일 | 변경 내용 |
|------|-----------|
| `conversational_ad_provider.dart` | `_saveBonusToServer()` 추가 → impression/click 즉시 RPC |
| `conversational_ad_widget.dart` | `_handleAdComplete` → `isRewardedAd: true` 고정 (RPC 중복 방지) |
| `ad_trigger_service.dart` | `depletedRewardTokensVideo`(35,000) / `depletedRewardTokensNative`(21,000) 상수 추가 |
...
| 버그 | 원인 | 수정 |
|------|------|------|
| `bonus_tokens=0` (86 impression인데) | "대화 재개" 미클릭 시 RPC 미호출 | impression 시 즉시 `_saveBonusToServer` |
| `depletedRewardTokensVideo/Native` 미존재 | 머지된 위젯이 참조하나 상수 없음 | ad_trigger_service.dart에 추가 |

## 9. 향후 TODO

- [ ] `rewarded_tokens_earned` 간헐적 0 모니터링 (trackRewarded 비동기 타이밍)
- [ ] integration_test 빌드 에러 수정 (플러그인 설정)
- [ ] AdMob eCPM 모니터링: 실제 수익 확인 후 interval 조정
- [ ] Firebase A/B 테스트: interval 2 vs 3 비교
