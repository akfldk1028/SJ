...
// 상극 관계 (내가 극하는 오행)
const GEUK: { [k: string]: string } = {
  '목': '토', '토': '수', '수': '화', '화': '금', '금': '목',
};

// 나를 극하는 오행
const PI_GEUK: { [k: string]: string } = {
  '목': '금', '금': '화', '화': '수', '수': '토', '토': '목',
};

// 오행→천간 매핑
const OHENG_TO_GAN: { [k: string]: { yang: string; yin: string } } = {
  '목': { yang: '갑', yin: '을' },
  '화': { yang: '병', yin: '정' },
  '토': { yang: '무', yin: '기' },
  '금': { yang: '경', yin: '신' },
...
export function getSpouseStar(ilgan: string, gender: string): SpouseResult {
  ...
  if (isMale) {
    // 남자: 배우자 = 재성 = 내가 극하는 오행
    const spouseOheng = GEUK[ganInfo.oheng];
    ...
    return {
      ...
      pyeonstar: `편재: ${pyeon}`,
      jeongstar: `정재: ${jeong}`,
      explanation: `${ilgan}${ganInfo.oheng} 남자 → ${ganInfo.oheng}이 극하는 ${spouseOheng}이 배우자성. 편재=${pyeon}, 정재=${jeong}. ⚠️ 관성(${PI_GEUK[ganInfo.oheng]})은 남자에게 직장/상사이지 배우자가 아님!`,
    };
  } else {
    // 여자: 배우자 = 관성 = 나를 극하는 오행
    const spouseOheng = PI_GEUK[ganInfo.oheng];
    const spouseGans = OHENG_TO_GAN[spouseOheng];
    const pyeon = ganInfo.yinyang === '양' ? spouseGans.yang : spouseGans.yin;
    const jeong = ganInfo.yinyang === '양' ? spouseGans.yin : spouseGans.yang;
    ...
    return {
      ...
      pyeonstar: `편관: ${pyeon}`,
      jeongstar: `정관: ${jeong}`,
      explanation: `${ilgan}${ganInfo.oheng} 여자 → ${spouseOheng}이 ${ganInfo.oheng}을 극하므로 관성이 배우자. 편관=${pyeon}, 정관=${jeong}. ⚠️ 재성(${GEUK[ganInfo.oheng]})은 여자에게 재물/아버지이지 배우자가 아님!`,
    };
  }
}
