    ...
    <ZodiacElementBackground elementName={calculation.identity.elementName}>
      <div className="mx-auto flex min-h-screen w-full max-w-md flex-col px-4 py-5 text-white sm:px-5 lg:max-w-4xl lg:py-7">
        <header className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase text-white/60">{eyebrow}</p>
            <h1 className="mt-1 text-[1.35rem] font-bold leading-8">{title}</h1>
          </div>
...
export function ProfileSummary({
  ...
  return (
    <section className="mt-5 rounded-lg border border-white/15 bg-white/[0.94] p-4 text-slate-950 shadow-xl shadow-black/15">
      <div className="flex items-center gap-3">
        <div className={`flex size-12 items-center justify-center rounded-md ${data.calculation.identity.colorClass} text-xl shadow-inner`}>
          {data.calculation.identity.animalEmoji}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-3">
            <h2 className="truncate text-lg font-bold">{data.profile.display_name}</h2>
            <span className="shrink-0 rounded-md bg-slate-950 px-2 py-1 text-[11px] font-bold text-white">
              {data.calculation.identity.ganjiHanja}
            </span>
          </div>
...
export function PillarCard({
  ...
  return (
    <div className={`min-w-0 rounded-lg border ${isDayMaster ? "border-amber-300 bg-amber-50 shadow-[0_0_0_1px_rgba(251,191,36,0.35)]" : "border-slate-200 bg-white"} px-2 py-3 text-center shadow-sm`}>
      <p className={`text-[11px] font-bold ${isDayMaster ? "text-amber-700" : "text-slate-500"}`}>{label}</p>
...
function StemBranchBox({
  ...
  return (
    <div className={`rounded-md border ${ohengStyle[element].border} bg-gradient-to-b from-white to-slate-50 px-1.5 py-2`}>
      <div className={`mx-auto mb-1 size-2 rounded-full ${ohengStyle[element].bg}`} />
      <p className="text-[1.7rem] font-black leading-8 text-slate-950">{hanja ?? value}</p>
      <p className="mt-1 text-[11px] font-semibold text-slate-600">{value} · {ohengKorean[element]}</p>
    </div>
  );
}
...
export function ReadonlyJsonCard({
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white/[0.96] p-4 text-slate-950 shadow-xl shadow-black/10">
      <h3 className="text-sm font-bold">{title}</h3>
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-4 text-slate-950">
      ...
      <div className="grid grid-cols-5 gap-1.5 sm:gap-2">
        {values.map(({ key, value }) => {
          ...
          return (
            <div className="rounded-md bg-slate-100 p-2 text-center ring-1 ring-slate-200" key={key}>
